...
int main(void)
{
	...
	printf("(1 ~ 15)%dA\t%dB\t%dC\t%dD\n",A, B, C, D);
	printf("(16~ 30)%dA\t%dB\t%dC\t%dD\n",AA, BB, CC, DD);
	printf("前A + 后D的总数(%d)\n", A+DD);
	printf("前B + 后C的总数(%d)\n", B+CC);
	printf("前C + 后B的总数(%d)\n", C+BB);
	printf("前D + 后A的总数(%d)\n", D+AA);

	if((A+DD)>(B+CC)&&(A+DD)>(C+BB)&&(A+DD)>(D+AA))
	{
		printf("You is 红色\n");
		printf("红色是行动者！\n");
	}
	else if((B+CC)>(A+DD)&&(B+CC)>(C+BB)&&(B+CC)>(D+AA))
	{
		printf("You is 蓝色\n");
		printf("蓝色是思想者！\n");
	}
	else if((C+BB)>(A+DD)&&(C+BB)>(B+CC)&&(C+BB)>(D+AA))
	{
		printf("You is 黄色\n");
		printf("黄色是领导者！\n");
	}
	else if((D+AA)>(A+DD)&&(D+AA)>(C+BB)&&(D+AA)>(B+CC))
	{
		printf("You is 绿色\n");
		printf("绿色是和平者\n");
	}
